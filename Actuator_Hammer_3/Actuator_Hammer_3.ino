...
void Retrieve_Hammer() {
  ...
  while (digitalRead(LIMITER_SWITCH_PIN) == LOW) {
    digitalWrite(RETRIEVE_RELAY_PIN, LOW);    //Energise Retrieving hammer
    digitalWrite(ACTIVATE_RELAY_PIN, HIGH);
  }
  //Hold the hammer there
  digitalWrite(RETRIEVE_RELAY_PIN, LOW);    //Energise Retrieving hammer
  digitalWrite(ACTIVATE_RELAY_PIN, LOW);
...
void loop() {
  ...
  if (Ultrasonic1.DidCrossThreshold()) {
    ...
    digitalWrite(ACTIVATE_RELAY_PIN, LOW);  //SMASH
